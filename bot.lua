...
chats = {}
day = 86400
bot_id = 390067362 -- [[محل قرار گیری آیدی اکانت ربات]]
sudo_users = {145064636} -- [[محل قرار گیری آیدی سودو ها]] + [[مدیر ربات]]
bot_owner = {145064636} -- [[ محل قرار گیری آیدی مدیر اصلی ربات ]]
-----------------------------------------------------------------------------------------------
---------------
-- Start Functions --
---------------
      ...
      if msg_type == 'MSG:Photo' then
        if not is_mod(msg.sender_user_id_, msg.chat_id_) then
          ...
          if not caption_text then
            ...
            if capion _text:match("کیر")or caption_text:match("کس")or caption_text:match("جنده")or caption_text:match("کون")or caption_text:match("کوس")or caption_text:match("[Kk]os")or caption_text:match("[Kk]on")or caption_text:match("[Ss]ex")or caption_text:match("[Kk]ir") then
             if database:get('bot:fohsh:mute'..msg.chat_id_) then
              local id = msg.id_
              local msgs = {[0] = id}
              local chat = msg.chat_id_
              delete_msg(chat,msgs)
              print("Deleted [Lock] [fohsh] [Photo] [NS8]")
            end
          end
        end
       end
